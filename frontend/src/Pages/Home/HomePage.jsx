...
export default function HomePage() {
  ...
  return (
    <main className="bg-[#fcfaf5] text-[#22211e]">
      ...
      <section className="mx-auto max-w-7xl scroll-mt-20 px-5 py-12 sm:px-10 sm:py-16 lg:px-6 lg:py-20" id="categories">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-12 sm:px-10 sm:pb-16 lg:px-6 lg:pb-20">
...
function Hero({ hero, term, onTermChange, onSearch }) {
  return (
    <section className="relative isolate flex min-h-[calc(100dvh-64px)] items-center overflow-hidden bg-cover bg-center px-5 py-16 sm:min-h-[580px] sm:px-10 sm:py-20 lg:px-[7%]" style={{ backgroundImage: `linear-gradient(90deg, rgba(27,22,20,.76), rgba(27,22,20,.14)), url('${hero.image}')` }}>
      <div className="max-w-3xl text-white">
        <p className="mb-4 text-xs font-bold tracking-[.18em] text-[#ffd8cf]">{hero.eyebrow}</p>
        <h1 className="font-['Playfair_Display'] text-[clamp(2.5rem,11vw,3.25rem)] leading-[1.08] font-semibold sm:text-6xl lg:text-7xl">{hero.title} <i>{hero.titleEmphasis}</i></h1>
        <p className="mt-6 max-w-xl text-base leading-7 text-white/90 sm:text-lg">{hero.description}</p>
        <form className="mt-8 flex max-w-2xl flex-col gap-2 rounded-xl bg-white p-2 text-[#706d65] shadow-xl sm:flex-row sm:items-center sm:rounded-md" onSubmit={onSearch}>
...
function AboutSection({ about }) {
  return (
    <section className="mx-5 mb-12 grid max-w-6xl scroll-mt-20 overflow-hidden rounded-xl bg-[#f2e5df] md:mx-auto md:mb-16 md:grid-cols-2 md:rounded-none lg:mb-20" id="our-story">
      <img className="h-72 w-full object-cover md:h-full" src={about.image} alt="Couple celebrating their wedding" />
      <div className="p-6 sm:p-12 lg:px-16 lg:py-20">
